...
static void read_and_write_test_read_handler_write_done(
    read_and_write_test_state* state, int write_done_state) {
  gpr_log(GPR_DEBUG, "Write handler done");
  gpr_mu_lock(g_mu);
  state->write_done = write_done_state;
  GRPC_LOG_IF_ERROR("pollset_kick", grpc_pollset_kick(g_pollset, nullptr));
  gpr_mu_unlock(g_mu);
}
...
static void write_scheduler(void* data, grpc_error_handle /* error */) {
  ...
  if (state->write_ep == nullptr) {
    read_and_write_test_read_handler_write_done(state, 1);
    return;
  }
...
static void read_and_write_test_write_handler(void* data,
  ...
  gpr_log(GPR_DEBUG, "Write handler done");
  read_and_write_test_read_handler_write_done(state, 1 + error.ok());
}
